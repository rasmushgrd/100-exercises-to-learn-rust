    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let first_handle = tokio::spawn(reply_client(first, Arc::clone(&reply)));
    let second_handle = tokio::spawn(reply_client(second, reply));
    let _ = tokio::join!(first_handle, second_handle);
}

async fn reply_client<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
